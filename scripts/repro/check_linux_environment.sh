...
OUTPUT=""
if [[ "${1:-}" == "--output" ]]; then
    [[ $# -eq 2 ]] || { echo "usage: $0 [--output] <output.json>" >&2; exit 2; }
    OUTPUT="$2"
elif [[ $# -eq 1 ]]; then
    OUTPUT="$1"
else
    echo "usage: $0 [--output] <output.json>" >&2
    exit 2
fi
...
"${PYTHON_EXECUTABLE}" - "${REPOSITORY_ROOT}" "${OUTPUT}" <<'PY'
...
def tool(name, expected_version=None):
    ...
    return {"status": status, "path": value, "expected_version": expected_version, "observed_version": None, "version_status": "UNVERIFIED" if value and not rejected else "NOT_CHECKED"}
...
payload = {
  ...
  "tools": tools,
  "policy": {"mnt_c_or_windows_executables_rejected": all(not ((v["path"] or "").lower().endswith(".exe") or (v["path"] or "").replace("\\", "/").lower().startswith("/mnt/c/")) for v in tools.values() if v["status"] != "REJECTED"), "missing_or_unverified_production_tools_status": "INCOMPLETE", "external_tools_executed_for_version_probe": False},
