...
## 本轮任务：中网格 rear_outlet_1 计算域修正与二阶 RANS 重新验收

状态：执行中。中网格原二阶 lineage 在 `segment_0034` 至 `segment_0036` 连续显示 `rear_outlet_1` 局部法向 Mach 从 `0.6616` 降至 `0.5072`，且相对质量不平衡仍约 `4.55e-3`；该 lineage 保留为合法 FAIL/PENDING 诊断证据，不再用于正式接受。

### 授权范围与执行顺序

- 使用 `segment_0036` 的显式解、网格及 SHA-256 定位亚声速面片，区分平面出口、锥形外边界、壁面交线及数值异常；诊断必须输出坐标、面积、face/node 编号与几何成员关系。
- 优先在 `geometry/derived` 构造延长后的计算域，使真实出口移至经小规模验证为全超声速的截面；不得修改原始 STEP、实体壁面、内部测量面、设计工况、参考量、SST `V2003m` 或猜测背压。
- 若诊断证明 `rear_outlet_1` 当前包含应属于 farfield 的锥形边界，必须通过可审计几何特征重新分类，不得按 Gmsh tag 或为通过门值而删除样本。
- 先完成派生几何、surface catalog、marker 重匹配、topology smoke 和小规模 RANS 出口验证；仅小规模出口门 PASS 后，才重新生成800万至1200万单元、冻结60层的生产中网格。
- 新中网格必须重新通过单元数、`min Scaled Jacobian >= 0.01`、阈下/负体积/非正 Jacobian/非有限质量/marker 遗漏/共享接口错误均为0、Prism/Tet共形及SU2只读检查。
- 网格拓扑变化后，中网格必须独立从零完成一阶检查点 RANS；禁止使用旧中网格或粗网格 restart。一阶 PASS 后方可进行500步MUSCL平滑切换和二阶检查点验收。
- 二阶仍要求连续3个完整窗口，并满足载荷0.5%、力矩1%、内部流量及总压恢复0.5%、质量不平衡`<=1e-3`、最大y+<1、两出口无回流/反向流且最小法向Mach>1，以及SU2/pvbatch返回码0和无非物理解。
- 最终证据必须绑定Git、几何、marker、网格、配置和restart SHA-256；全部证据回传校验后才更新 `DESIGN_MEDIUM_RANS_SECOND_ORDER=PASS`。在此之前敏感性比较与其余工况继续HOLD。

## 本轮任务：设计点粗—中两级正式 SST RANS 与网格敏感性分析

状态：执行中。仅对设计工况 `M50_H21_A8_B0` 在已验收的约500万粗网格和9,582,917单元系统加密中网格上运行两阶段 SST RANS；不生成细网格，不运行其余四工况。本阶段结果只能称为两级网格敏感性分析，不能称为三网格 GCI 或严格网格无关性验证。
...
- `PRODUCTION_CFD = IN_PROGRESS`

### 当前执行子阶段：提交控制代码并启动中网格独立 RANS

- 提交前审查当前求解控制、诊断与网格族兼容修改，运行完整测试并安全扫描；仅提交源码、测试、脚本和PLAN，不提交CAD、runs、`config/tools.json`、远程证据或大型工件。
- push 后要求本地HEAD、GitHub `linux-autodl-candidate` 与AutoDL detached checkout三方SHA完全一致，且AutoDL源码工作区干净；粗网格最终证据目录与证据包只读保留，不覆盖。
...
- 唯一初场为粗网格一阶 PASS 的 `segment_0099/restart.dat`，固定 SHA-256 为 `b12a76e4c4ab25ce9ed4720111268be051438b04c9e6ba19ac68b9457f1b163f`；禁止按目录新旧搜索 restart。
- 首段采用 `MUSCL_FLOW/TURB=YES`、`RAMP_MUSCL=YES`，在500步内由一阶平滑切换到完整二阶；其后关闭 ramp 并保持完整二阶。SST `V2003m`、Roe/限制器、边界条件、参考量、marker、网格和湍流入口保持不变。
- 每段最多500步、最近300步轻量判定、段末写出并验证二进制 restart；仅候选窗口、最终验收或异常时运行完整 pvbatch，连续3个完整诊断窗口通过后立即停止。
- 二阶硬门：主要力相对跨度不超过0.5%，主要力矩归一化跨度不超过1%，内部质量流量和总压恢复跨窗口变化均不超过0.5%，相对全局质量不平衡不超过`1e-3`，最大 y+<1且超限面积比例为0，两个后出口反向流量和回流面积均为0且最小法向Mach>1；返回码为0且无NaN/Inf或非物理解。
- 最终证据必须包含完整 restart lineage、逐段配置/history/restart/manifest/窗口统计/stdout/stderr，以及最终解、六分量、截面、质量守恒、y+、出口、资源报告和证据包 SHA-256。

### 目标与验收标准

- 正式 RANS 改为检查点驱动：每段最多500步，段末必须校验restart并记录SHA-256；每段读取最近300步history。粗网格一阶门采用用户于2026-08-05冻结的合同：返回码0；无NaN/Inf/负压力/负密度；残差无持续恶化；相对质量不平衡不超过`5e-3`；主要力窗口跨度不超过2%；力矩采用允许区间内更严格的3%；窗口内部及相邻窗口均不得持续单向漂移；连续2个完整诊断窗口满足即提前PASS。仅`PENDING`允许启动下一段，保留总步数上限但禁止为跑满上限忽略提前停止条件；全部统计、原因、restart lineage及SHA-256写入checkpoint manifest。
- 粗、中计算必须绑定各自显式 PASS 网格 manifest 和 SHA-256；除网格外，真实 BREP、marker/边界条件、设计工况、21 km 美国标准大气、显式 SST 模型及修正选项、无滑移绝热壁面、参考量、坐标系、出口冻结合同、数值格式、CFL 策略、湍流入口、监控量及后处理公式完全一致。
- 每套网格独立完成低 CFL 一阶启动及显式 restart 绑定的二阶计算；禁止粗到中 restart 复用或目录搜索。SU2/MPI 返回码、history、restart、体/表面解、命令、版本、资源和 SHA-256 必须完整。
- 正式长算前在粗网格进行 8/16/24 ranks 短标定，按每迭代耗时、峰值内存、分区开销、MPI 稳定性选择统一进程数；不启用 CUDA。
- 每套网格硬门包括：无 NaN/Inf、负密度/压力/温度或异常湍流变量；最大 y+<1 且 y+>1 面积比例为0；全局相对质量不平衡和两个出口门满足冻结合同；两个出口无回流/反向质量流且最小法向 Mach>1；pvbatch 返回0。
- 二阶最终稳定窗口必须同时检查残差、六分量、内部测量面流量/总压恢复、全局质量守恒及出口量，无持续单向漂移或未经处置的周期振荡；一阶结果不作最终交付。
- 只有粗、中单套门均 PASS 后才比较六分量、内部截面量、y+、质量守恒及两个出口量。主要力、内部流量、总压恢复及截面平均量采用2%工程容差，力矩采用3%；近零量使用配置定义的绝对或归一化容差，不使用失真的相对误差。
- 全部主要指标满足两级容差后，才可设置 `TWO_LEVEL_MESH_SENSITIVITY=PASS`、冻结系统加密中网格并将其余四工况置为 READY；仍明确缺少细网格，禁止声称 GCI 或严格网格无关性。
- 远程每阶段使用唯一目录，证据包下载后在本机复算 SHA-256 并重新解析；最终更新 PLAN、运行本地质量门、审查并保持私有 CAD、runs、`config/tools.json` 和大型工件不进入 Git。

### 粗网格一阶完成记录（2026-08-05）

- 固定设计工况 `M50_H21_A8_B0`、4,965,647 单元粗网格、显式 `RANS/SST/V2003m` 和一阶格式；24 MPI ranks、CUDA 未启用。
- 按每段500步、最近300步窗口执行至 `segment_0099`，累计49,500步，未超过50,000步上限。
- 用户冻结的一阶机器合同为：主要力跨度不超过2%，主要力矩跨度不超过3%，相对质量不平衡不超过`5e-3`，残差无持续恶化，无NaN/Inf及非正密度/压力/温度，两个出口硬门通过，连续两个完整窗口合格。内部测量面流量和总压恢复稳定性在一阶保留为监控量，不作为一阶restart接受硬门；它们仍是二阶正式验收门。
- `segment_0098` 为连续合格窗口1，`segment_0099` 为连续合格窗口2；最终 `checkpoint_manifest.json` 状态为`PASS`、判定原因列表为空。
- 最终一阶restart为 `segment_0099/restart.dat`，SHA-256为`b12a76e4c4ab25ce9ed4720111268be051438b04c9e6ba19ac68b9457f1b163f`；`segment_0099`输入restart与`segment_0098`输出哈希完全一致。
- 远程验收报告为 `runs/rans_formal/design_coarse_checkpoint_v2_20260804T174715Z/first_order/first_order_acceptance.json`，`overall=PASS`。该一阶结果只作为二阶初场，不作最终气动力交付。

### 粗网格二阶完成记录（2026-08-05）

- 唯一初场为一阶 PASS restart SHA-256 `b12a76e4c4ab25ce9ed4720111268be051438b04c9e6ba19ac68b9457f1b163f`；使用500步平滑 MUSCL ramp 后保持完整二阶、SST `V2003m`、Roe/限制器及全部物理合同不变。
- CFL=1.0 lineage 出现载荷振荡并最终 NaN；从一阶 PASS 重新建立 CFL=0.1 干净 lineage。质量守恒门附近的恢复分支均显式保留，任何 FAIL restart 均未作为后续输入。
- 最终合法 lineage 在 CFL=0.005/0.001 受控窗口取得连续3个完整诊断窗口 PASS；最终 restart SHA-256 为 `6c5791c7782cdee7c0d431a7b46b94db410bf250dbc94d4dcc880bf2ff5ce199`。
- 最终相对全局质量不平衡为`9.885907956211552e-4`，最大 y+为`0.3215551972389221`且 y+>1面积比例为0；两个后出口最小法向Mach为`1.0736860383194349/4.491374825739249`，反向质量流量和回流面积比例均为0。
- 最终内部测量面净质量流量为`1.143657142190286 kg/s`，总压恢复为`0.04443667349845694`；SU2/MPI与pvbatch返回码均为0，无NaN/Inf或非物理解。
- 远程最终报告位于 `runs/rans_formal/design_coarse_second_order_cfl010_20260805T181500Z/second_order/final_acceptance/`。证据包已回传至 `remote_evidence/coarse_second_order_pass_20260805T193500Z/`，本机复算SHA-256为`cd747409616769247931612dbb6e1a1b30b85ea02539863872686ecb98a4b249`并重新解析为PASS。
...
- 四个体区域的实际特征尺寸必须从粗到中系统减小，区域中/粗比与冻结`0.794`一致；报告P05/P50/P95、样本数、实际比值和容差判定。
- 壁面表面三角形必须实际切向加密，三角形数量和棱柱柱列增加，切向特征尺寸减小；不得用不变的冻结表面三角化误报PASS。
- 60层、首层高度`2.7645771273320875e-7 m`、增长率`1.2`及局部质量修复原则保持不变；任何重新生成的中网格仍须独立通过`minSJ>=0.01`及全部生产质量门。
- 内流道、头部、入口、转弯段、出口上游和近体核心必须有实际加密证据；farfield仍为最疏区域，marker语义与共享接口关系保持一致。
- 只有四区比例、壁面切向、内流道、近体、远场、边界层、marker/interface及同族判定全部PASS，才可设置`COARSE_MEDIUM_SYSTEMATIC_REFINEMENT=PASS`并授权约2000万细网格；本轮不生成细网格、不运行RANS。
- 审计器、JSON/Markdown、命令、日志、输入SHA-256及回传证据完整；全部本地回归、代码审查和三方Git SHA一致。
...
- `COARSE_REGION_REVIEW = PASS`
- `COARSE_REGION_STRUCTURE = FROZEN`
- `PRODUCTION_MEDIUM_MESH = AUTHORIZED_NOT_BUILT`
- `PRODUCTION_FINE_MESH = HOLD`
- `RANS = DIAGNOSTIC_L6_PASS`
- `PRODUCTION_CFD = HOLD`
...
- 尺寸场合同冻结为 farfield/transition/near-body/internal-passage 粗级尺寸 `0.0542217/0.0361478/0.0240985/0.0165677 m`，bounds 逐级嵌套，过渡宽度为正且由线性距离场与最小值连续组合；四个区域在网格族中不移动。
- AutoDL 对粗网格本体的只读统计中，四区 Tet4 特征尺度中位数为 `0.0368541/0.0247716/0.0164372/0.0117414 m`，严格递减；全部4,685,327个四面体质心均位于farfield范围内。
- 280,320个Prism6的壁面法向厚度中位数为`2.76986e-7 m`，小于内流道核心中位尺度；壁面切向三角化、60层、首层高度、增长率与Pilot局部修复参数保持冻结不变。
- 粗网格质量仍为最小Prism Scaled Jacobian `0.010098248712453189`、所有硬失败计数0，MSH哈希未变；实测审计报告回传后SHA-256为`4697c1834179af61c70d12325f0527479c5a78d9cb746af7341e36d579667da9`。
- 中级统一尺寸系数为`0.794`，目标1000万、合法范围800万至1200万，按实测粗网格核心三次方投影为9,640,375单元。授权只允许生成；生成结果通过独立质量门前禁止RANS，细网格继续HOLD。
...
- 网格族物理区域固定为 farfield、壁面/内流道、近体核心与过渡区；farfield 最疏，壁面、内流道和近体核心加密，区域物理范围在粗/中/细三级保持不变，尺寸统一按 `1.000/0.794/0.630` 缩放并平滑过渡。
- 粗网格目标500万三维单元，合法范围475万至525万；中/细目标仍为1000万/2000万，本轮不生成。后续网格尺度比和GCI必须使用实际单元数计算。
- 保留 Pilot 审计通过并冻结的60层边界层结构、首层高度、增长率及局部方向/层高修复参数（同时满足不少于15层的客户要求）；不得以减层调整单元量。边界层与四面体核心共形，farfield、出口、内部测量面及共享接口不得生成边界层。
- 475万至525万的总量校准只能调整壁面切向尺寸、近体核心区、内流道、过渡区和远场体网格尺寸；若资源或校准证据表明60层下无法进入该范围，必须保留校准与资源报告，禁止自动回退为15层。
- 最小 Scaled Jacobian `>=0.01`、阈下单元0、负体积0、非正 Jacobian 0、非有限质量0、marker遗漏0、共享接口错误0；任一门失败不得运行SU2求解或进入中/细网格。
- 必须成功写出 `mesh.msh` 与 `mesh.su2`，SU2仅执行网格读取/预处理检查且返回0，不运行Euler或RANS；全部输入、命令、版本、资源、输出和SHA-256进入唯一新证据目录。
- 粗网格加密区域及质量门通过后，才允许规划中网格；本轮保持 `RANS=DIAGNOSTIC_L6_PASS` 与 `PRODUCTION_CFD=HOLD`。
- 完整本地测试、代码审查和 `git diff --check` 必须PASS；AutoDL固定提交运行、证据打包回传并由本机复算SHA-256和重新解析验收报告。
